...
int iStopLightSensor = 30;
int iStopColorSensor = 18;
int iRotateAngle = 150;
int iRotateSpeed = 25;
int iStraightSpeed = 60;
int sleepTime = 0;
string strColorSensor = "Go right";
string strLightSensor = "Go left";
...
task main()
{
	...
	while(true)
	{
		...
		if(getColorReflected(ColorSensor) >= iStopColorSensor && SensorValue(LightSensor) >= iStopLightSensor)
		{
			...
			sleep(sleepTime);
			setMotorSyncEncoder(LeftTire, RightTire, 0, 360, iStraightSpeed);
		...
		{
			PrintStats();
			displayBigTextLine(10, strColorSensor);
			sleep(sleepTime);
		...
		{
			PrintStats();
			displayBigTextLine(10, strLightSensor);
			sleep(sleepTime);
